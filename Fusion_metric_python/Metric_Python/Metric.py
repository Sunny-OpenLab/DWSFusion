...
from Fusion_metric_python.Metric_Python.Qabf import get_Qabf
from Fusion_metric_python.Metric_Python.Nabf import get_Nabf
import math
from Fusion_metric_python.Metric_Python.ssim import ssim, ms_ssim
...
def CC_function(A,B,F):
    rAF = np.sum((A - np.mean(A)) * (F - np.mean(F))) / (np.sqrt(np.sum((A - np.mean(A)) ** 2) * np.sum((F - np.mean(F)) ** 2)) + 1e-8)
    rBF = np.sum((B - np.mean(B)) * (F - np.mean(F))) / (np.sqrt(np.sum((B - np.mean(B)) ** 2) * np.sum((F - np.mean(F)) ** 2)) + 1e-8)
    CC = np.mean([rAF, rBF])
    return CC
...
def SSIM_function(A, B, F):
    ssim_A = ssim(A, F)
    ssim_B = ssim(B, F)
    SSIM = 0.5 * ssim_A + 0.5 * ssim_B
    return SSIM.item()

def MS_SSIM_function(A, B, F):
    ssim_A = ms_ssim(A, F)
    ssim_B = ms_ssim(B, F)
    MS_SSIM = 0.5 * ssim_A + 0.5 * ssim_B
    return MS_SSIM.item()
