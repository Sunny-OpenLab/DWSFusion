...
def get_Nabf(I1, I2, f):
    ...
    maskAF1 = (gA == 0) | (gF == 0)
    maskAF2 = (gA > gF)
    gAF[~maskAF1] = np.where(maskAF2, gF / (gA+1e-8), gA / (gF+1e-8))[~maskAF1]
    maskBF1 = (gB == 0) | (gF == 0)
    maskBF2 = (gB > gF)
    gBF[~maskBF1] = np.where(maskBF2, gF / (gB+1e-8), gB / (gF+1e-8))[~maskBF1]
    aA = np.where((gvA == 0) & (ghA == 0), 0, np.arctan(gvA / (ghA+1e-8)))
    aB = np.where((gvB == 0) & (ghB == 0), 0, np.arctan(gvB / (ghB+1e-8)))
    aF = np.where((gvF == 0) & (ghF == 0), 0, np.arctan(gvF / (ghF+1e-8)))
