...
class AdaptiveDilatedConv(ModulatedDeformConv2d):
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'OMNI_ATT1') and hasattr(self, 'OMNI_ATT2'):
            ...
            adaptive_weight = adaptive_weight.reshape(-1, self.in_channels // self.groups, self.kernel_size[0], self.kernel_size[1])
...
if __name__ == '__main__':
    x = torch.rand(32, 4, 16, 16).cuda()
    m = AdaptiveDilatedConv(in_channels=4, out_channels=1, kernel_size=5).cuda()
    m.eval()
    y = m(x)
    print(y.shape)
